...
public abstract class ClientSocket implements Runnable
{
	...
	protected ExecutorService mainThread = Executors.newSingleThreadExecutor(ExecutorUtil.makeName("clientsocketmainpool"));
	protected ScheduledExecutorService scheduledThread = Executors.newSingleThreadScheduledExecutor(ExecutorUtil.makeName("clientsocketschedulepool"));
	...
	protected ClientSocket(String hostname, int port, Class<? extends ChannelInitializer<SocketChannel>> initializerClass)
	{
	...
	public void dispatcher(Message message)
	{
		...
		if (processes[mainNo][subNo] != null)
		{
			...
			if (timestamp2 - timestamp1 > 1000)
				logger.info("handle clientsocket dispatcher protocol-[{},{}] too long = {}", mainNo, subNo, timestamp2 - timestamp1);
		}
		else
		{
